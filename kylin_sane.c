 ...
 */
void get_option_colors(SANE_Handle sane_handle, int optnum)
{
 ...
 */
void get_option_sources(SANE_Handle sane_handle, int optnum)
{
    SANE_Status status = 0;
    const SANE_Option_Descriptor *opt;
    int i = 0;
	...
	if (opt) {
        ...
        switch(optnum)
        {
            case 2:
                ...
                val_string_color = "Color";

                get_option_colors(device, optnum);
                status = set_option_colors(device, optnum, "Color");
                if(status != SANE_STATUS_GOOD)
                {
            ...
            case 3:
                ...
                status = set_option_sources(device, optnum, "Transparency Adapter");
                break;
            ...
            case 6:
                ...
                if(opt->constraint_type == SANE_CONSTRAINT_WORD_LIST)
                {
                    ...
                    set_option_resolutions(device, optnum, 150);
                }
                break;
